...
function render(){
  const lang=getLang();
  const names=lang==='ar'?`${ex.name1_ar} & ${ex.name2_ar}`:`${ex.name1_en} & ${ex.name2_en}`;
  const venue=lang==='ar'?ex.venue_ar:ex.venue_en;
  const date=new Date(ex.date).toLocaleDateString(lang==='ar'?'ar-EG':'en-GB',{dateStyle:'long'});
  document.title=`${names} — WEDORA`;
  document.querySelector('#exampleMount').innerHTML=`
    <button class="inv-music" data-lang-toggle style="inset-inline-end:auto;inset-inline-start:16px">${lang==='ar'?'EN':'ع'}</button>
    <main class="invite-main template-${tm.slug}" style="--accent:${tm.accent};--ibg:${tm.background}">
      <section class="invite-cover">
        <div class="invite-cover-art"></div>
        <div class="invite-cover-meta"><small>OUR CELEBRATION</small><h1>${names}</h1><small>${date}</small></div>
      </section>
...
document.addEventListener('click',e=>{if(e.target.closest('[data-lang-toggle]')){setLanguage(getLang()==='ar'?'en':'ar');render()}});
render();
