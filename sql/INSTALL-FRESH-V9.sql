...
-- ================= 04 ACCOUNT SUBSCRIPTION =================

-- =============================================================
-- WEDORA V6.5 — ACCOUNT SUBSCRIPTION + DIRECT PUBLISH
-- Run ONCE after the V6 / V6.2 setup.
-- This restores the intended product model:
-- one active account plan -> publish invitations without paying again.
-- =============================================================
...
-- Recognise customers whose payment was already approved in V6.
-- Existing subscriptions are never overwritten by this backfill.
with latest_approved as (
  select distinct on(o.user_id)
    o.user_id,o.plan_id,o.id as order_id,
    coalesce(o.reviewed_at,o.created_at,now()) as started_at,
    p.duration_months,p.is_lifetime
  from public.orders o
  join public.plans p on p.id=o.plan_id
  where o.status='approved'
  order by o.user_id,coalesce(o.reviewed_at,o.created_at) desc
)
insert into public.subscriptions(user_id,plan_id,status,started_at,current_period_end,is_lifetime,source_order_id)
select
  x.user_id,x.plan_id,'active'::public.subscription_status,x.started_at,
  case when x.is_lifetime then null else x.started_at+make_interval(months=>coalesce(x.duration_months,12)) end,
  x.is_lifetime,x.order_id
from latest_approved x
where x.is_lifetime or x.started_at+make_interval(months=>coalesce(x.duration_months,12))>now()
on conflict(user_id) do nothing;

create or replace function public.get_active_subscription()
returns table(
  id uuid,user_id uuid,plan_id uuid,status public.subscription_status,
  started_at timestamptz,current_period_end timestamptz,is_lifetime boolean,
  plan_slug text,plan_name_ar text,plan_name_en text,plan_features jsonb
)
language sql stable security definer set search_path=public as $$
  select s.id,s.user_id,s.plan_id,s.status,s.started_at,s.current_period_end,s.is_lifetime,
         p.slug,p.name_ar,p.name_en,p.features
...
create or replace function public.publish_with_subscription(p_invitation_id uuid)
...
begin
  ...
  set status='active',plan_id=s.plan_id,is_lifetime=s.is_lifetime,is_trial=false,
      active_until=case when s.is_lifetime then null else s.current_period_end end,
      published_at=coalesce(published_at,now()),updated_at=now()
  where id=p_invitation_id;
end $$;

-- Going forward, approving one payment activates/extends the CUSTOMER ACCOUNT,
-- and also publishes the invitation attached to that payment.
...
begin
  ...
  select * into existing from public.subscriptions where user_id=o.user_id;

  -- Never downgrade an already-active lifetime account.
  if existing.id is not null and existing.status='active' and existing.is_lifetime then
    new_start:=existing.started_at;new_end:=null;final_plan:=existing.plan_id;final_lifetime:=true;
  elsif p.is_lifetime then
    new_start:=coalesce(existing.started_at,now());new_end:=null;final_plan:=p.id;final_lifetime:=true;
  else
    final_plan:=p.id;final_lifetime:=false;
    if existing.id is not null and existing.status='active' and not existing.is_lifetime
       and existing.current_period_end is not null and existing.current_period_end>now() then
      new_start:=existing.started_at;
      new_end:=existing.current_period_end+make_interval(months=>coalesce(p.duration_months,12));
    else
      new_start:=now();
      new_end:=now()+make_interval(months=>coalesce(p.duration_months,12));
    end if;
  end if;

  insert into public.subscriptions(user_id,plan_id,status,started_at,current_period_end,is_lifetime,source_order_id,updated_at)
  values(o.user_id,final_plan,'active',new_start,new_end,final_lifetime,o.id,now())
  on conflict(user_id) do update set
    plan_id=excluded.plan_id,status='active',started_at=excluded.started_at,
    current_period_end=excluded.current_period_end,is_lifetime=excluded.is_lifetime,
  ...
  update public.invitations
  set status='active',plan_id=final_plan,is_lifetime=final_lifetime,is_trial=false,
      active_until=case when final_lifetime then null else new_end end,
      published_at=coalesce(published_at,now()),updated_at=now()
  where id=o.invitation_id;
end $$;
...
grant execute on function public.publish_with_subscription(uuid) to authenticated;
grant execute on function public.approve_order(uuid) to authenticated;

-- Quick verification for the currently logged-in data from SQL Editor:
select s.user_id,p.email,p.full_name,pl.name_ar as plan,s.status,s.is_lifetime,s.current_period_end
from public.subscriptions s
...
-- ================= 05 PREMIUM TEMPLATES =================

-- WEDORA V7 — Premium template migration
-- Run once in Supabase SQL Editor on an existing V6.x database.
